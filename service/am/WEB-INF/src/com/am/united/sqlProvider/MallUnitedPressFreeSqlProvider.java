 ...
 * 区县联合会会费管理
 */
public class MallUnitedPressFreeSqlProvider implements SqlProvider {
	...
	public String getSql(ActionContext ac) {
		...
		String querSQL="SELECT d1.f_name,frecord.*  "+
					" ,to_char(frecord.free_date,'yyyy-MM') AS ffree_date"+
					"  FROM  mall_united_press_free AS frecord "+
					"  LEFT JOIN mall_federation AS d1 "+ 
					"  ON d1.id=frecord.united_press_free_id WHERE 1=1  ";
		
		//收费机构为当前登录人所在机构
		querSQL+=" AND frecord.f_id LIKE '"+ac.getVisitor().getUser().getOrgId()+"%' ";
		
		Row queryRow=FastUnit.getQueryRow(ac,"am_bdp", "mall_united_press_free.query");
		...
		if(queryRow!=null){
			...
			if(!Checker.isEmpty(fName)){
				querSQL+=" AND d1.f_name LIKE '"+fName+"%' ";
			}
		...
		querSQL+=" ORDER BY d1.f_name,frecord.free_date DESC ";
