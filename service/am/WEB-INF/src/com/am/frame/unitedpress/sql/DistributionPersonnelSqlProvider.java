 ...
 *@create 2016年4月30日
 *@version
 *说明：配送人员管理SqlProvider
 */
public class DistributionPersonnelSqlProvider implements SqlProvider{
	...
	public String getSql(ActionContext ac) {
		...
		String sql =  "SELECT am.membername,trim(to_char(COALESCE(ur.buy_price/100),'99999999999990D99')) AS money,ur.* "
				+ " FROM mall_distribution_personnel AS ur "
				...
				+ " AND ur.zone_id IN ( "
				+ " SELECT zone_id FROM mall_distribution_center "
				+ " WHERE 1=1  AND orgid LIKE '"+orgid+"' "
				+ " ) ";
		
		
		//获取查询单元
		Row queryRow = FastUnit.getQueryRow(ac, "am_bdp","mall_distribution_personnel.query");
