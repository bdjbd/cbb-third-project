...
public class SelectUserSqlProvider implements SqlProvider {
	...
	public String getSql(ActionContext ac) {
		...
		try {
			...
			Row query = FastUnit.getQueryRow(ac, "wwd", "select.user");
			...
			if ("0".equals(account_belong)) {// 私有公众帐号
				sql = "  select member_code as member_code,nickname as nickname,mem_name ||'/'||phone as namephone,gender as gender,age as age,"
						+ "group_id as groupid,public_id as public_id,data_status as data_status from WS_MEMBER where data_status='1' and orgid= '"
						+ orgid + "' ";
				if (query != null) {
				...
				sql = "select b.member_code as member_code,b.nickname as nickname,b.mem_name ||'/'||b.phone as namephone,b.gender as gender,"
						...
						+ "on a.member_code=b.member_code where a.data_status='1' and a.care_orgid = '"
						+ orgid + "' ";
				if (query != null) {
					nickname = query.get("nickname");
					group_id = query.get("groupid");
