 ...
 */
<!--  http://localhost:8080/am_bdp/choice_member.do?orgcode=cro_carmanager.form.orgcode&index=0&clear=am_bdp.choice_member.query  -->


function openDialog(s,popupUnitName){
    var index=0;
    var elementName = s.get('orgcode');
    if(s.parentNode.parentNode.id!=""){
        index = s.parentNode.parentNode.id;
    }
    window.open('/am_bdp/'+popupUnitName+'.do?orgcode='+elementName+'&index='+index+'' +
...
function confirmData(unitName){
    //当前弹出层list页面所有数据
    var selectArray=$$('input[orgcode=_s_'+unitName+']');
    //当前弹出层list页面所有数据的id
    var idArray=document.getElementsByName(""+unitName+".id");
    //获取隐藏域中的存入session中的值
    var orgcode=document.getElementsByName(""+unitName+".orgcode")[0].value;
    //索引
    var index=document.getElementsByName(""+unitName+".index")[0].value;
    var finished = false;
    for(var i=0;i<selectArray.length;i++){
        if(selectArray[i].value=='1'){
            window.top.opener.document.getElementsByName(""+orgcode+"")[index].value=idArray[i].value;
            finished = true;
            break;
        }
    }
