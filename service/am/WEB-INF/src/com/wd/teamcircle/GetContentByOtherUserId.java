...
/**
 * 按照分页，获取指定用户发布的班组圈子列表内容
 * */
public class GetContentByOtherUserId implements ICuai {

	/**
	 * 获取指定用户发布的班组圈子列表内容
	 * @param jsonArray 参数{"当前用户";"当前页数";"要查看用户的id"}
	 * */
	@Override
	public JSONArray doAction(JSONArray jsonArray) {
		...
		try {
			...
			otherUserId=jsonArray.getString(2);//要查看用户的id
		...
		String sql="select a.id,publisher,b.username,idea,a.createtime,a.value,c.name,photopath,b.sex,ROW_NUMBER() OVER(order by a.createtime desc)" +
				...
				" where a.publisher=b.userid and a.value=c.value and a.publisher='"+otherUserId+"' and id in" +
