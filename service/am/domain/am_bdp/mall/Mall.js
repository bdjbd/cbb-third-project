...
function newcase(){
    window.parent.document.getElement('iframe[id=mall_commodityclass.frame.mall_commodity_class_info_frame]').src
    ='/am_bdp/mall_commodity_map.do?m=s&mall_commodityclass.form.id='+th.ae.s.id;
}

//添加终端菜单信息
function addTerminalMenu(){
     window.parent.document.getElement('iframe[id=mall_commodityclass.frame.mall_commodity_class_info_frame]').src
     ='/am_bdp/mall_commodity_map.do?m=a&upid='+th.ae.s.id;
}
//修改终端信息
function editTerminalMenu(){
     window.parent.document.getElement('iframe[id=mall_commodityclass.frame.mall_commodity_class_info_frame]').src
     ='/am_bdp/mall_commodity_map.do?m=e&mall_commodityclass.form.id='+th.ae.s.id;
}
...
function deleteTerminalMenu(){
	if (confirm("确定要删除'"+th.ae.s.innerText+"'吗？")){
		...
		var str = "/am_bdp/mall_commodityclass.tree/tree.do?clazzId="+th.ae.s.id;
       doAjaxSubmit(str);
	}
}
