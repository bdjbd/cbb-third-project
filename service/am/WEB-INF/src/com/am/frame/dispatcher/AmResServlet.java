...
/**  
 * @author  wz  
 * @date 创建时间：2016年3月24日 上午10:52:16  
 * @version 1.0 
 * @parameter 
 * */
public class AmResServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		if(CheckTonken.getSingleton().tonken(request,action))
		{
	...
	//			Utils.Log(tag,"执行 | " + action + " | 完成");
		...
		{
			PrintWriter out = new PrintWriter(response.getOutputStream());
			String result = "{\"CODE\":\"40006\",\"TonkenMSG\":\"令牌无效!\"}";
			try {
				//执行接口方法
				AmResServlet rs = new AmResServlet();
	//			Utils.Log(tag,"开始执行 | " + action + " | ");
