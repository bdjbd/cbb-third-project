 ...
 * 分类保存Action
 */
public class CommdityClassSaveAction extends DefaultAction {
	...
	public void doAction(DB db, ActionContext ac) throws Exception {
				...
				//分类标签表
				Table markTable=ac.getTable("mall_cmdclassmarks");
				...
				//保存文件路径  pcdetailsimage   bdp_pcdetailsimage  MALL_COMMODITY 多文件 详情图片
				String detailsimage=Utils.getFastUnitFilePath("MALL_COMMODITYCLASS", "bdp_pcdetailsimage", classId);
						
				if(!Checker.isEmpty(detailsimage)){
					detailsimage=detailsimage.substring(0, detailsimage.length()-1);
					String updateDetailsSql="UPDATE MALL_COMMODITYCLASS  SET pcdetailsimage='"
						+detailsimage+"'  WHERE id='"+classId+"' ";
							
					db.execute(updateDetailsSql);
				}
