 ...
 * @version 
 * 说明:<br />
 *  农技协联合会初始化 农技协每个省，市，区只能有一个
 */
public class ATAFAccountInitAction extends GroupInitActionAbstract {
	...
	public JSONObject initOrg(DB db, String orgId) throws Exception {
		...
		//1,检查是否有农技协，如果有，不初始化
		MapList checkMap=checkOrgTypeExist(db, orgId, "mall_federation");
		...
		if(!Checker.isEmpty(checkMap)){
			...
			//农技协联合会
			addOrg(db, orgId, "mall_federation");
