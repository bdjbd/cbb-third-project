	...
	public ActionContext execute(ActionContext ac) throws Exception {
		...
		if(!Checker.isEmpty(map)){
			...
			if("1".equals(dataStatus)){//起草--》启用
				updateSQL="UPDATE p2p_eterpelectticket SET datastatus='2' WHERE id='"+eleId+"'";
			}
			
			if("2".equalsIgnoreCase(dataStatus)){//2 启用-->3
				updateSQL="UPDATE p2p_eterpelectticket SET datastatus='3' WHERE id='"+eleId+"'";
			}
		...
		ac.getActionResult().setUrl("/wwd/p2p_eterpelectticket.do?m=s&clear=wwd.p2p_eterpelectticket.query");
