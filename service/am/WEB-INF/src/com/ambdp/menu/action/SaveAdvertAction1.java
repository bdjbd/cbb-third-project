 ...
 * 
 */
public class SaveAdvertAction1 extends DefaultAction {
	final Logger logger = Logger.getLogger(SaveAdvertAction1.class);
	@Override
	public void doAction(DB db, ActionContext ac) throws Exception {
		// super.doAction(db, ac);
		Table table = ac.getTable("am_advert");
		db.save(table);
		String menucode = ac.getRequestParameter("am_advert1.form.am_mobliemenuid");
		System.err.println("获得menucode》》》"+menucode);
		//内容组件Id
		String amContentid = ac.getRequestParameter("am_advert1.form.am_contentid");
		//类型
		String contentUrl = ac.getRequestParameter("am_advert1.form.content_url");		
		String id = table.getRows().get(0).getValue("id");
		ac.setSessionAttribute("am_bdp.am_advert1.form.id", id);
