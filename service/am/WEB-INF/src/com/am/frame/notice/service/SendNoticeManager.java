 ...
 * 发送通知管理类    
 * @author mac
 *	张少飞 2017-09-16  修改第241行判断条件、 第 274行判断条件  当发送的设备集合的数量不足950时，直接全部发送，详见方法内注释
 */
...
//			XingePushNotice push = new XingePushNotice();
			...
			for (int i = 0; i <= android_i; i++) {
				...
				//安卓设备总台数(注意大集合的size，下标已确定) 
				if(list_size<950)  //建议将条件改为list_size<950  ，当大集合的数量不足950时，直接全部发送，此时只有一道循环
				{
			...
			//所要发送的ios设备集合，必须分成 ios_i 段，分段发送
			...
			for (int i = 0; i <= android_i; i++) {
				...
				if(ios_list_size<950)   
				{
					//当发送的ios数量不足950台时，不用截取，直接发送
