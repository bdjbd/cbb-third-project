 ...
 * 按照分页，获取指定圈子的内容
 * */
public class GetContentByCircleId implements ICuai {

	/**
	 * 获取指定圈子的内容
	 * @param jsonArray 参数{"圈子Id";"当前页数"}
	 * */
	@Override
	public JSONArray doAction(JSONArray jsonArray) {
		...
		try {
			cid=jsonArray.getString(0);//指定圈子id
		...
		String sql="select a.id,publisher,b.username,idea,a.createtime,a.value,c.name,photopath,b.sex,ROW_NUMBER() OVER(order by a.createtime desc)" +
				...
				" (select id from tc_purview where cid='"+cid+"') order by a.createtime desc" +
