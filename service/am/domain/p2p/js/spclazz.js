...
function setRole(){
  ...
  var id=th.ae.s.id;
  if(id=='1'){
      window.parent.document.getElement('iframe[id=p2p_spclazz.e2]').src='about:blank';
  }else{
      window.parent.document.getElement('iframe[id=p2p_spclazz.e2]').src='/wwd/p2p_serviceandcommodity.list.do?m=e&p2p_serviceandcommodity.list.commodityclazz='+th.ae.s.id+"&comdytype=3";
  }
}

function newcase(){
	var comdytype=document.getElementsByName("goodsorts.tree.comdytype")[0].value;
    window.parent.document.getElement('iframe[id=goodsorts.frame.e2]').src='/wwd/ws_commodity_edit.do?m=e&parent_id='+th.ae.s.id+"&comdytype=3";
}

//添加终端菜单信息
function addTerminalMenu(){
	var comdytype=document.getElementsByName("goodsorts.tree.comdytype")[0].value;
     window.parent.document.getElement('iframe[id=goodsorts.frame.e2]').src='/wwd/ws_commodity_edit.do?m=a&parent_id='+th.ae.s.id+"&comdytype=3";
}
//修改终端信息
function editTerminalMenu(){
	var comdytype=document.getElementsByName("goodsorts.tree.comdytype")[0].value;
     window.parent.document.getElement('iframe[id=goodsorts.frame.e2]').src='/wwd/ws_commodity_edit.do?m=e&parent_id='+th.ae.s.id+"&comdytype="+comdytype;
}
//删除终端信息
