...
public class P2pMemeberAuthenSqlProvider implements SqlProvider {
			...
			if (listr.size() > 0) {
				...
				if ("0".equals(account_belong)) {// 私有公众帐号
					sql = "  select *,openid, headimgurl,member_code as member_code,nickname as nickname,mem_name ||'/'||phone as namephone,gender as gender,age as age," +
							"group_id as groupid,public_id as public_id,data_status as data_status,provice||city AS area  from WS_MEMBER where orgid= '"+orgid+"'  AND isceritfy!=0";
					if (query != null) {
					...
					sql = "select b.openid, b.headimgurl,b.member_code as member_code,b.nickname as nickname,b.mem_name ||'/'||b.phone as namephone,b.gender as gender,b.provice||b.city AS area ,"
