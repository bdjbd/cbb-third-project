...
//		String isUpdate[] = ac.getRequestParameters("_s_ws_commodity_name.list");
		...
		for (int i = 0; i < isUpdate.length; i++) {
			if("1".equals(isUpdate[i])) {
				if("2".equals(data_status[i])){
					ac.getActionResult().addErrorMessage("第"+(i+1)+"行，商品已经上架，请重新选择！");
					return;
				}
		...
		//批量更新
		String sql = "update ws_commodity_name set data_status=2,added_time=CURRENT_TIMESTAMP where comdity_id = ? ";

		db.executeBatch(sql, list,new int[]{Type.INTEGER});
		ac.getActionResult().addSuccessMessage("上架成功");
