 ...
 *  服务中心，市，区县各个只能有一个
 */
public class ServiceCenterInitAction extends GroupInitActionAbstract {
	...
	public JSONObject initOrg(DB db, String orgId) throws Exception {
		...
		//1,检查是否有农技协，如果有，不初始化
		MapList checkMap=checkOrgTypeExist(db, orgId, "mall_service_center");
		...
		if(!Checker.isEmpty(checkMap)){
			...
			//农技协联合会
			addOrg(db, orgId, "mall_service_center");
